...
const getSalesPageInfo = async (req, res) => {
    ...
    try {
        ...
        const recentSuccessOrders = await Order.find({ status: "delivered", payment_status: "paid" })
        .sort({createdAt:-1}).limit(5).populate("user_id");
        ...
        return res.status(STATUS.SUCCESS.OK).json({ stats ,recentSuccessOrders });
...
export { getSalesPageInfo, getSalesOverview, getSalesOverviewCustom, getExcelSalesReport, getPdfSalesReport };